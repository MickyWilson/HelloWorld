...
public class ComplicatedExample 
{
	...
	private  long millisToPauseForFileLock;
	private  long millisToSwapFileForUnlocking;
...
outerLoop: 
        	while(!closeProcessingThread || !fileProcessingQueue.isEmpty())
        	{
	            try
	            {
...
innerLoop: 		while(true)
				{
                	...
                	catch(FileNotFoundException fnfe)
                	{
                		...
                		try
                		{
                			Thread.sleep(millisToPauseForFileLock);
                		}
                		catch(InterruptedException ie)
                		{
                		}
                		if((System.currentTimeMillis() - startTime) > millisToSwapFileForUnlocking)
                		{
    ...
    public static ComplicatedExample watchDirectory(String dirName, boolean recursive) throws InvalidPathException, IOException, Exception{
        try{
            Path dir = Paths.get(dirName);
            final ComplicatedExample watchDir = new ComplicatedExample(dir, recursive);
     ...
     */
    private ComplicatedExample(Path dir, boolean recursive) throws IOException {
        fileProcessingQueue = new ArrayBlockingQueue<String>(fileQueueSize, false);
        processedFileQueue = new ArrayBlockingQueue<String>(fileQueueSize, false);
